...
$sql="SELECT name,district FROM quote WHERE district='$dis'";
$quote=mysqli_query($conn,$sql);
$i=-1;
while($data = mysqli_fetch_array($quote))
{
    $i++;
	$n[$i]=$data['name'];
	$d[$i]=$data['district'];
        ...
        <div class="wrapper" style="min-height: 636px;">
            <div class="container">
                ...
                <div class="blocks">
					...
					for($k=0;$k<=$i;$k++)
					{
                      	echo '<div class="col-md-4 col-xs-12   wow bounceInDown" data-wow-delay="0.2s">';
                        echo  '<div class="block">';
                        echo        '<div class="row">';
                        echo            '<div class="col-md-12 col-xs-3 text-center">';
                        echo                '<div class="icon">';
                        echo                    "<img src='src/images/nomini/$n[$k].jpg' class='img-responsive img-circle' alt='' />";
                        echo                '</div>';
                        echo            '</div>';
                        echo            '<div class="col-md-12 col-xs-9">';
                        echo                "<div class='title'> $n[$k] <br>DISTRICT : $d[$k] </div>";
						echo				 '<a class="btn-action btn-round btn-gradient wow fadeIn" data-wow-delay="0.2s" href="src/php/part1.php">Vote</a>';
                        echo            '</div>';
                        echo        '</div>';
                        echo    '</div>';
                        echo '</div>';
